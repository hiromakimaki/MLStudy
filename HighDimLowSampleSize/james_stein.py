...
def main():
    ...
    for base_num in [0, 0.9, 1, 1.1]:
        ls_mse, js_mse = simulation(base_num, dims, n_simulations)
        plt.plot(dims, np.zeros(len(dims)), linestyle='--', c='black')
        plt.plot(dims, ls_mse, label='Least squares')
        plt.plot(dims, js_mse, label='James-Stein')
